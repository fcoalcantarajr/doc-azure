...
def test_doc_evidence_accepts_the_exact_line_and_source_hash(tmp_path):
    page = tmp_path / "page.md"
    page.write_text("# Bug\nPossui 8 estados\n", encoding="utf-8")
    digest = hashlib.sha256(page.read_bytes()).hexdigest()
    verify_doc_line(page, 2, "Possui 8 estados", digest)
...
def test_doc_evidence_rejects_a_line_outside_the_source_range(tmp_path):
    page = tmp_path / "page.md"
...
def test_json_pointer_rejects_non_ascii_array_indices_with_the_segment():
    with pytest.raises(EvidenceError, match="٠"):
        resolve_json_pointer(["first"], "/٠")
